let test_tls_authenticator_fails_closed_without_ca_bundle () =
  Alcotest.(check bool) "missing CA paths are rejected"
    true
    (match Obs_loki_tls.authenticator
             ~ca_paths:["/obs-loki-eio/does/not/exist/ca-certificates.crt"] () with
     | Error `No_system_ca_bundle -> true
     | _ -> false)

let test_tls_authenticator_ignores_invalid_ca_bundle () =
  let path = Filename.temp_file "obs-loki-invalid-ca" ".pem" in
  Fun.protect
    (fun () ->
       ...
       Alcotest.(check bool) "invalid CA file is rejected"
         true
         (match Obs_loki_tls.authenticator ~ca_paths:[path] () with
          | Error `No_system_ca_bundle -> true
          | _ -> false))
...
let test_tls_wrapper_returns_typed_error_without_ca_bundle () =
  Alcotest.(check bool) "wrapper setup returns typed CA error"
    true
    (match Obs_loki_tls.make_https_wrapper
             ~ca_paths:["/obs-loki-eio/does/not/exist/ca-certificates.crt"] () with
     | Error `No_system_ca_bundle -> true
     | _ -> false)
...
(* Regression test ported from aws-eio's Aws_tls fix: tls-eio's handshake
   ...
   by the system CA bundle) through Obs_loki_tls.https_for_uri and expects
   it to fail on certificate trust, not on the unseeded-RNG error. *)
...
let test_https_handshake_fails_on_cert_not_on_rng () =
  ...
  match Obs_loki_tls.https_for_uri dummy_uri with
  | Error e -> Alcotest.fail ("expected an https wrapper, got: " ^ Obs_loki_tls.error_to_string e)
...
(* Regression test ported from aws-eio's fix for a second blocker: a bare
   ...
   Atomic.t cache (see Obs_loki_tls.default_https_wrapper). This exercises
   that fixed path under real concurrent-domain contention against a cache
   forced cold immediately beforehand, so it's exercising the actual
...
let test_concurrent_domains_never_see_lazy_undefined () =
  ...
  Atomic.set Obs_loki_tls.default_https_wrapper_cache None;
  ...
  let domains =
    List.init domain_count (fun _ ->
        Domain.spawn (fun () ->
            ...
            try
              ignore (Obs_loki_tls.https_for_uri uri : (Obs_loki_tls.https_wrapper option, Obs_loki_tls.error) result);
              Ok ()
